...
var chancfg, lftp, what, err
    ...
    case 'echo':
        if(lftp && chancfg && chancfg.id){// use strong shell quotes
            return lftp.stdin.write("echo '" + chancfg.id + "'\n", 'utf8', on_done)
        } else {
    ...
    case 'quit':
        ...
        return lftp.stdin.write('quit\n', 'utf8', on_done)//???if long process of up/down????
    case 'start':
        if(!lftp && chancfg && (!chancfg.sts || 'quit' == chancfg.sts.slice(0, 4))){
    ...
    return next(new Error(err || '!no such `do` action'))

    function on_done(){
        log('chs sts:', chancfg.sts)
        api.wes.broadcast('wes4store',{
            store: 'lftpd',
            data:[ ret.data = chancfg ]
        })
        return res.json(ret)
    }
}
...
var cwd = local.cfg.data_path + chancfg.id// -> $PWD/data/suprolftpd/GLOB
    ...
    var lftp
        ...
        lftp = lftps[chancfg.id] = local.require.cp.spawn(
        ...
        if(!lftp.pid || lftp.exitCode){
            err = new Error('!FATAL spawn `lftp` exit code: ' + lftp.exitCode)
            if(cb) return cb(err)
            throw err
        }
        ...
        function on_lftp_close(code){
            log('$ `lftp` stop code: ', String(code))
