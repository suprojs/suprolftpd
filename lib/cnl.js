...
function lftp_recv_mkcb(mod, obj, filename){
    return function on_read(err, str){
        ...
        function on_event(err){
            return !err && local.require.fs.unlink(filename, lftp_loop_done)
        }
    }
}
...
'
        ...
        function(st, lftp, chancfg){
            ...
            if(chunck && ~chunck.indexOf('zzzzzzz.zzz')){
            ...
            // isn't an error but warning about remote peer isn't downloading
                if(~chunck.indexOf('No such file')){
                    return// skip completely
                }
                if(~chunck.indexOf('Failure')){
            ...
            } else return // skip partial strings (usually heads of them)
            wes(chancfg)
