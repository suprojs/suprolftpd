...
var chancfg, lftp, what, err
    ...
    */
    ...
    case 'api.lftp.send':
        ...
        return setImmediate(on_done)// no async calls here
    case 'api.lftp.on':
        api.lftp.on('suprolftpd', function(json){
log(' lftp recv:', json)
        })
        return setImmediate(on_done)// no async calls here
    default:
...
// ~~~~
// TEST  -> $PWD/data/suprolftpd/TEST/upload/_123123123-userman.TEST
// GLOB <= $PWD/data/suprolftpd/TEST/download/_123123123-userman.TEST
// ----
// GLOB: sends to all (i.e all configured channels), receives from all objects
// ~~~~
// GLOB -> $PWD/data/suprolftpd/TEST/upload/_123123123-userman.TEST
// GLOB -> $PWD/data/suprolftpd/TES2/upload/_123123123-userman.TES2
// TEST <= $PWD/data/suprolftpd/TEST/download/_123123123-userman.TEST
// TES2 <= $PWD/data/suprolftpd/TES2/download/_123123123-userman.TES2
function lftp_loop(){
    ...
    for(obj in lftps) if(lftps[obj]){// object: one channel; GLOB: all channels
        for(m in local.app_modules){
            ...
            data.json && local.require.fs.writeFile(
                local.cfg.data_path + obj + fname + m + '.' + obj,
                data.json,
...
' mput  -c -O .proc -E * && ' +
'(source .mv.lftp ; !rm .mv.lftp) && echo OK || (!sed -n p .mv.lftp ; echo ERR)\n\
',
            'utf8', lftp_loop_done
        )
   ...
   ,d = new Date
    ...
    var err, dir, st, sh
        ...
        function(st, lftp, chancfg, sh){
            ...
            if('__' != chunck.slice(0, 2)){
                return
            }
            //first message form lftpd.sh is: `echo "__
            //,- lftpd.sh --`
            chunck = st == 'up' ? 'up' : 'down'
//# Error messages on STDOUT must have prefix 'ERR'
//# lftp> open connection
//# lftp> cd to working directory making it if not there yet
...
cd '      + chunck + 'load          && echo @PWD: && pwd | sed s_/[^@]*@_//_' +
                     '              || echo ERR cd\n\
'
            sh.stdin.write(chunck)// issue initial channel commands
        };})(st, lftp, chancfg, sh))
        ...
        function(st, lftp, chancfg){
            ...
            if(chunck && ~chunck.indexOf('Failure (.proc/zzzzzzz.zzz)')){
            //        'mv: Access failed: Failure (.proc/zzzzzzz.zzz)'
            // isn't an error but warning about remote peer isn't downloading
                chunck = 'zzzz'// keep length == 4 to skip error status
            }
            if(chunck.length > 4 && // filter too small chunck
               !~chunck.indexOf('no files found') &&
               !~chunck.indexOf('No such file')){// if fatal error change status
                ...
                chancfg.sts = (st == 'up'? 'e'  + chancfg.sts[1]:
                          chancfg.sts[0] + 'e') + chancfg.sts[2]+
                          '|"'+ st + '" err: ' + chunck
            } else if(chunck){// reset after possible partial first chunk
                chancfg.sts = (st == 'up'? 'r'  + chancfg.sts[1]:
                          chancfg.sts[0] + 'r') + chancfg.sts[2]+
                          '|"'+ st + '" ' + (4 == chunck.length ? chunck : 'skip')
            }
            wes(chancfg)
            lftp.log.write(chunck)
        };})(st, lftp, chancfg))
