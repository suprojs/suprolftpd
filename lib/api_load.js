...
var url = require('url')
   ...
   ,path = require('path')
   ,app_modules = {/* modname: { cb: cb, send:[ ], json:'' }*/}
   ,ui_api, local, d
    ...
    var m = app_modules[name]
        ...
        if(!m){
            app_modules[name] = { cb:cb, send: [ ], json:'' }
        } else if(!m.cb){
            m.cb = cb
        } else {// this is programming error:
    ...
    var d = { mod:mod,json:json }
       ,m
        ...
        if((m = app_modules[mod])){
            m.send.push(d)
        } else {
            app_modules[mod] = { cb: void 0, send:[d], json:'' }
        }
