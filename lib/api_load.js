...
var url = require('url')
   ...
   ,path = require('path')
   ,app_modules = {// transport link of data from sending peer to receiveing one:
   /* modname: { on: cb(obj, send, doEventCleanup(err)), send:[ ], str:'' } */
    }//                      ~~~~\___,---<<---<<---,___/~~~~~; str is cache
   ,ui_api, local, d
    ...
    load_api()
   /*
    * Application API is set/reset if reloaded in `cnl.js` like so:
    * api.lftp = {
    *     OBJ: local.cfg.OBJ,
    ...
    **/
    ...
    read_private_config()// private (from `git`) config used in specific tasks
    ...
    var ret = { data: null, err: null }// for errors: `return next(err)`
       ,call//                                                        \.../
