/*
 * Common API load / reload boilerplate
 */
...
var url = require('url')
   ...
   ,ui_api, local
    ...
    ui_api = {// add custom URLs here
        'api': null,
        'dev': load_api
    }
    load_api()

    cfg = ui_api['api'](null, api, local)// dynamic config direct API call

    cfg.data_path = path.normalize(
    //        $PWD/app_modules/suprolftpd/lib/ -> $PWD/data/suprolftpd/
        __dirname + '/../../..' + (cfg.data_path || '/data/suprolftpd/')
    )
    if(cfg.bin){//      $PWD/app_modules/suprolftpd/lib/ -> $PWD/bin/lftp.exe
